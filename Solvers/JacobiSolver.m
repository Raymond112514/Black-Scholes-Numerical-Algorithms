% Black Scholes PDE (for put options) implicit method solver using Jacobi method
% Input:
...
function [price, elapsedTime] = JacobiSolver(S0, K, r, T, sigma, Smax, M, N, text, verbose)
    ...
    for j = N:-1:1
        ...
        for iter = 1:maxIter
            ...
            xnew = zeros(M-1, 1);
            for i = 1:M-1
                sum1 = dot(coeff(i, 1:i - 1), xold(1:i - 1));
                sum2 = dot(coeff(i, i + 1:end), xold(i + 1:end));
                xnew(i) = (target(i) - sum1 - sum2) / coeff(i, i);
            end
            mesh(2:M, j) = xnew;
